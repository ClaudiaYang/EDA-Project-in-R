...
This analysis is motivated by a need to investigate the disparities and inequalities in educational outcomes across different regions and demographics. Through exploring the potential relationships between variables mentioned above, we want to discover some pattern that might highlight areas in need of education policy intervention. By evaluating the extent to which these resources correlate with students’ academic success, the educational stakeholders could also gain some data-driven insights for improving public education. 
...
hist(score$Total.Math, 
     ...
     border = "black")
```
We plotted a histogram of total math scores to understand the distribution of scores across the sample. The histogram shows how frequently different score ranges occur.

From the histogram, we can observe that the total math scores are not evenly distributed, but rather concentrated around a central range. The peak, occurring around scores between 480 and 520, suggests that the majority of students achieved scores close to the average.  This central peak, along with a few lower and higher outliers, indicates that this histogram is a normal distribution, with a slight skew towards the higher end. Only a small portion of students scored below 450 or above 550.
...
```
We plotted a pie chart to represent the distribution of test takers by gender. The chart is divided into two segments, each showing the proportion of male and female participants in the test. The segments are labeled "Female" and "Male," with pink and light blue colors respectively.   
